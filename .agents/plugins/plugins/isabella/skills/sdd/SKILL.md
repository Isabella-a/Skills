...
| 4 | `fases/fase4_investigacao.md` | Investiga os padrões do repositório e módulos similares | Fase 2 concluída (paralela com a Fase 3) |
| 5 | `fases/fase5_confirmacao.md` | Deriva a lista de specs (granularidade + reuso) e confirma o escopo técnico | Fases 3 e 4 concluídas |
| 6 | `fases/fase6_geracao.md` | Gera a pasta `.specs/sdd-<slug>/` com todos os arquivos | Fase 5 concluída |
...
| `templates/implementacao.md` | Template para `implementacao.md` (Fase 6) |
| `templates/progresso.md` | Template para `progresso.md` (Fase 6) |
| `templates/spec.md` | Template para cada arquivo em `specs/` (Fase 6) |
...
| skill de grilling (ex.: `grill-me` ou `grill-with-docs`, de `mattpocock-skills`) | Entrevista o usuário em rounds até a ideia estar afiada (Fase 1) |
...
- **Skills externas de outro plugin** (`grill-me`/`grill-with-docs`, referenciada na Fase 1): no
  Claude Code vêm de `mattpocock-skills`; no Codex, instale-as com `npx skills@latest add
  mattpocock/skills`. Se não existir uma delas no runtime, conduza a entrevista de grilling você
  mesmo (várias rodadas de perguntas até a ideia estar afiada) em vez de pular a Fase 1.
...
5. **Leia `regras/qualidade.md` antes de escrever qualquer spec** na Fase 6.
6. **Nunca invente contrato, DTO, entidade ou schema.** Se a Fase 3 não retornou o shape real,
...
7. **A granularidade sai do perfil, não do seu gosto.** O campo "unidade de entrega" do
   `perfil.md` define o que é uma spec neste repositório; a Fase 5 aplica isso.
8. **A Fase 1 é obrigatória em toda invocação**, com ou sem ticket — a busca do ticket é o
   único passo opcional dela. Não pule para a Fase 2 achando a descrição "clara o suficiente":
