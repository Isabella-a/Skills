...
- **Register a KTLO** (support / routine / investigation / one-off data pull) — see the
  dedicated flow in section 4b

## Configuration

**Project Detection Strategy (Automatic):**

1. **Check skill config first**: Look for Jira configuration in `jira-config.md` (next to this `SKILL.md`)
...
1. Check if the skill folder has `jira-config.md` with Jira configuration
2. If found, extract and use: Project Key, Cloud ID, URL, Board URL, and issue type names
3. If not found:
...
**Note for skill users:** To configure this skill for your workspace, create `jira-config.md`
in the skill folder with your project details (see `jira-config.md` for the current project).

### MCP transport

This skill talks to Jira through the **Atlassian Rovo MCP server**, which must use the
**Streamable HTTP** transport at `https://mcp.atlassian.com/v1/mcp`.
...
### 4b. Registering a KTLO

When the user asks to register/log a **KTLO** ("cadastre um KTLO", "registra esse suporte",
"abre um KTLO de rotina"), it's a **Tarefa** with the custom fields **KTLO**, **Área**,
**Team**, and **Sprint** (always the active sprint) filled in — not a plain task. The field
IDs, the option tables, how to find the active sprint, and a known `issuetype` gotcha are all
documented in **`references/ktlo.md`**. Read that file before creating a KTLO.
...
- **Use detected configuration** - Read from `jira-config.md` (in the skill folder) or ask user
